...
En el cálculo de una variable, al calcular $\lim_{x \to a} f(x)$, el punto $x$ solo puede acercarse a $a$ por dos caminos: por la izquierda o por la derecha. En el cálculo multivariable, la situación es radicalmente más compleja: el punto $(x, y)$ puede acercarse a $(x_0, y_0)$ por \textbf{infinitos caminos distintos} — rectas, parábolas, espirales, etc. Esto hace que demostrar la existencia de un límite sea más exigente, y que detectar su inexistencia sea más sutil.

\subsection{Definición formal del límite}

\begin{mydefinition}{Límite de una función de dos variables}
Sea $f: D \subseteq \mathbb{R}^2 \to \mathbb{R}$ y sea $(x_0, y_0)$ un punto de acumulación del dominio $D$. Se dice que el \textbf{límite} de $f$ cuando $(x,y)$ tiende a $(x_0, y_0)$ es el número real $L$, y se escribe:
...
\begin{tikzpicture}[scale=1.2]
    ...
    % Punto (x,y) interior
    \filldraw[red] (1.2, 1.1) circle (1.5pt)
        node[right, font=\footnotesize, red] {$(x,y)$};

    \node[blue, font=\footnotesize] at (-1.2, -1.7) {Disco: $0 < \|(x,y)-(x_0,y_0)\| < \delta$};

    % ---- Flecha de mapeo ----
    \draw[->, thick, gray!70] (2.6, 0) -- (3.8, 0)
        node[midway, above, font=\footnotesize] {$f$};

    % ---- Eje z (derecha) ----
    \draw[->, gray] (4, -2) -- (4, 2.5) node[above] {$z$};

    % Intervalo (L-eps, L+eps)
    \draw[orange, very thick] (4, 0.3) -- (4, 1.3);
    \filldraw[orange] (4, 0.8) circle (2pt) node[right, font=\footnotesize] {$L$};
    \draw[dashed, orange] (3.7, 0.3) -- (4.3, 0.3)
        node[right, font=\footnotesize] {$L - \varepsilon$};
    \draw[dashed, orange] (3.7, 1.3) -- (4.3, 1.3)
        node[right, font=\footnotesize] {$L + \varepsilon$};

    % Valor f(x,y)
    \filldraw[red] (4, 0.95) circle (1.5pt)
        node[left, font=\footnotesize, red] {$f(x,y)$};

    \node[orange, font=\footnotesize] at (5.3, -1.5)
        {$|f(x,y) - L| < \varepsilon$};
\end{tikzpicture}
\end{center}

La definición exige que \textit{sin importar el camino} por el que $(x,y)$ se acerque a $(x_0,y_0)$, el valor $f(x,y)$ siempre deba converger al mismo $L$. Este es el requisito clave que distingue el límite multivariable del univariable.

\begin{mynote}{Generalización a $\mathbb{R}^n$}
...
Existen tres estrategias principales para evaluar (o refutar) un límite multivariable.
...
Este método se usa para \textbf{demostrar que un límite no existe}. Si se puede encontrar al menos dos trayectorias que se acercan al mismo punto pero producen valores límite distintos, entonces el límite no existe.
...
El límite no existe, aunque \textit{todas las rectas} hubieran dado el mismo resultado. Esto ilustra que pasar la prueba de rectas \textbf{no es suficiente} para garantizar la existencia del límite.
...
\begin{mytheorem}{Criterio polar}
Si al sustituir $x = r\cos\theta$, $y = r\sin\theta$ la expresión de $f$ se puede acotar como $|f| \leq g(r)$ donde $g(r) \to 0$ cuando $r \to 0^+$ \textbf{independientemente de $\theta$}, entonces:
\[ \lim_{(x,y)\to(0,0)} f(x,y) = 0 \]
Si la expresión resultante depende de $\theta$ de forma irreductible, el límite probablemente no existe (verificar con trayectorias).
\end{mytheorem}
...
El resultado \textbf{depende de $\theta$}: no converge a un valor único. El límite \textbf{no existe}.

\subsubsection{Teorema del emparedado (Squeeze Theorem)}
...
Una función $f: D \subseteq \mathbb{R}^2 \to \mathbb{R}$ es \textbf{continua} en el punto $(x_0, y_0) \in D$ si se satisfacen simultáneamente las tres condiciones:
\begin{enumerate}
...
\end{enumerate}
Si $f$ es continua en todos los puntos de $D$, se dice que $f$ es \textbf{continua en $D$}.
\end{mydefinition}
...
\begin{itemize}
    \item Todo \textbf{polinomio} $p(x,y) = \sum a_{ij} x^i y^j$.
    \item Toda \textbf{función racional} $p(x,y)/q(x,y)$ donde $q(x,y) \neq 0$.
    \item Las funciones \textbf{trigonométricas, exponenciales, logarítmicas y raíces} aplicadas a expresiones continuas.
    \item La \textbf{composición} de funciones continuas: si $g: \mathbb{R}^2 \to \mathbb{R}$ es continua y $\phi: \mathbb{R} \to \mathbb{R}$ es continua, entonces $\phi \circ g$ es continua.
\end{itemize}
\end{mytheorem}

\subsubsection{Extensión continua de una función}

Cuando una función presenta una indeterminación en un punto, puede ser posible \textbf{extenderla} de manera continua redefiniendo su valor en ese punto.
...
\begin{itemize}
    \item \textbf{No está definida:} $(x_0,y_0) \notin D$ (p.ej., denominador cero).
    \item \textbf{El límite no existe:} la función oscila o toma valores distintos según el camino.
    \item \textbf{El límite existe pero no coincide con $f(x_0,y_0)$:} discontinuidad evitable (se puede corregir).
\end{itemize}